...
// Función para reiniciar datos de muestra
async function resetOrganizations() {
  try {
    // Eliminar todas las organizaciones existentes
    await Organization.destroy({ where: {}, truncate: true });
    
    // Datos de muestra con logos
    const organizations = [
      {
    ...
    console.log('Datos de muestra actualizados correctamente con logos.');
  } catch (error) {
    console.error('Error al reiniciar datos de muestra:', error);
  } finally {
    process.exit();
  }
}
